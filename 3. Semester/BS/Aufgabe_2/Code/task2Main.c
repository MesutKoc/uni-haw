...
/*|-------------------------------------------------------------------
  ...
  |-------------------------------------------------------------------*/
int p1_stop = 0, p2_stop = 0, consumer_stop = 0, sleep_consumer = 2;
...
void* producing(void *pid, pthread_cond_t *restart, int *prod_stopped, char a_begin, char a_end, int sleep_time) {
	...
	//Kritischer Abschnitt
        ...
        //hier locken wir den mutex entweder für Producer_1 oder Producer_2
	pthread_mutex_lock(&rb_mutex);

        while(p_rb ->count >=MAX-1) {
...
/*|------------------------------------------------------------
  ...
  | Lösungsidee: http://stackoverflow.com/questions/7469139/what-is-equivalent-to-getch-getche-in-linux 
  | @return char
  |------------------------------------------------------------*/
char get_char() {
...
void *control(void *pid) {
	...
	while(running) {
		...
		switch (input) {
			...
			case 'c':
			case 'C':
					 consumer_stop = (consumer_stop == 0) ? 1 : 0;
					 printf("consumer = %d\n",consumer_stop);
					 pthread_cond_signal(&read_condvar);
					 break;
			case 'q':
			case 'Q':
					 printf("Alle laufenden Threads beenden!\n");
					 running=0;
					 p1_stop=1;
					 p2_stop=1;
					 consumer_stop=1;
...
int main(int argc, char* argv[]){
	...
	p_rb -> p_in = p_start; // Setzen von p_in Pointer im Ringpuffer auf p_start
	p_rb -> p_out = p_start; // Setzen von p_out Pointer im Ringpuffer auf p_start
	p_rb -> count = 0;	// Setzen vom count in Ringpuffer auf 0
	...
	//Blocken die non_running_condvar.
