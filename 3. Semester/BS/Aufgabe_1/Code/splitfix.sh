# ------------------------------------------------------------
# In diesem Fall legt man also fest, dass für die Ausführung des Skriptes 
# die Bash verwendet werden soll.
# ------------------------------------------------------------
#! /bin/bash
...
# ------------------------------------------------------------
# This variable debug for the debugging message
# ------------------------------------------------------------
s_debug=1
...
s_setsize() {
	...
	# Debugging Message
	#---------------------------------------------
	s_debug "Size wird gesetzt"
	#-----------------------------------------
	# Wenn nichts übergeben wird an Parameter
	#-----------------------------------------
	if [ $# -eq 0 ]; then
		echo 'Anzahl der File-Stuecke fehlt!'
		exit 1
	fi
	
	#-----------------------------------------
	# Gültige Zeichen finden
	#-----------------------------------------
	echo $1 | grep  "[^0-9]" > /dev/null 2>&1
	if [ "$?" -eq "0" ]; then
		echo "Ungültige Anzahl der File-Stuecke"
		exit 1
	fi
...
# ------------------------------------------------------------
# This function tests the -v debug and print the debug
# ------------------------------------------------------------
s_enable_debug() {
	s_debug = 0;
}
